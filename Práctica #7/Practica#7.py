...
df = pd.read_csv('coffee.csv', index_col='Date', parse_dates=True)
y = df['Close']
...
history = model.fit(train_seqs.reshape(-1, seq_len, 1), train_targets, epochs=50, batch_size=32, verbose=2, validation_data=(test_seqs.reshape(-1, seq_len, 1), test_targets))
